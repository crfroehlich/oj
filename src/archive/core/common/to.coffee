...
((OJ) ->
    ...
    tryGetNumber = (val) ->
      ...
      if OJ.is.number(val)
        ...
        tryGet = ((value) ->
          ...
          num = parseInt(value, 0)  unless OJ.is.number(num)
          num
        (val))
        ret = tryGet  if OJ.is.number(tryGet)
      ret
    'use strict'
    retVal = tryGetNumber(inputNum)
    unless OJ.is.number(retVal)
      retVal = tryGetNumber(defaultNum)
      retVal = Number.NaN  unless OJ.is.number(retVal)
    retVal
  ...
  OJ.to.register "vendorDomObject", (id) ->
    ret = null
    base = "#"
    base = ""  if id is "body"
    _$el = OJ["?"](base + id)
  ...
  OJ.to.register "vendorDomObjFromString", (html) ->
    ret = null
    _$el = OJ["?"](html)
    ret = _$el  if _$el
    ret
